...
class MyDataset(Dataset):
    def __init__(self, type, img_size, data_dir):
        self.name2label = {"15": 0, "16": 1, "17": 2, "18": 3, "19": 4, "20": 5, "21": 6, "22": 7, "23": 8, "24": 9,
                           "25": 10, "26": 11, "27": 12, "28": 13, "30": 14, "40": 15}
        self.img_size = img_size
        self.data_dir = data_dir
        self.data_list = list()

        for class_dir in range(15, 29):  # 已经分好了文件夹的数据集导入方式
    ...
    def __getitem__(self, item):  # 数据的序号
        ...
        # 获取前4个标签
        label = [1, 2, 3, 4]
        label[0] = self.name2label[os.path.basename(file).split('_')[1]]
        label[1] = self.name2label[os.path.basename(file).split('_')[2]]
        label[2] = self.name2label[os.path.basename(file).split('_')[3]]
        label[3] = self.name2label[os.path.basename(file).split('_')[4]]
