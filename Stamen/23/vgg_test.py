...
def val(test_dataset):
    ...
    for (images, labels, area) in test_dataset:
        ...
        if (outputs >= 0.5000 and labels != 1.0) or (outputs < 0.5000 and labels != 0.0):
            # 如果预测错误
            print(labels)  # 模型的标签
            print(test_dataset.data_list[step])  # 判断错误的图片
            source_path = test_dataset.data_list[step]
        ...
        # 精度
        if outputs >= 0.5000 and labels == 1.0:  # 正样本预测正确
            correct = correct + 1.0
        if outputs < 0.5000 and labels == 0.0:  # 负样本预测正确
            correct = correct + 1.0
...
if __name__ == '__main__':
    ...
    parser.add_argument('-img_path', type=str, default="train_dataset_genai_area_zuizhong",
                        help='images path')
    parser.add_argument('-gongkuang', type=str, default="抽油杆断脱", help='本分类器的工况类别')
    parser.add_argument('-new_img_path', type=str, default="./judge_false_add.area_gybz_23_sig(shang_chu50_xia_chu50)_jqss15",
                        help='new images path')
    parser.add_argument('-model_path', type=str,
                        default="E:\\aboutjiqixuexi\\cygdt_23\\checkpoint_wei_sig(shang_chu50_xia_chu50)_area\\epoch_5_train_dataset_genai_area_zuizhong-best-acc_0.9909722222222223.pth",
                        help='model path')
