...
def train(epoch, pbar):
    ...
    for step, (images, labels, area) in pbar:
        ...
        column = labels.unsqueeze(0)  # 真实标签
...
def val():
    ...
    for (images, labels, area) in test_loader:
        ...
        if outputs[:, 0] >= 0.5 and labels[:, 0] == 1.0:
            correct = correct + 1.0
        if outputs[:, 0] < 0.5 and labels[:, 0] == 0.0:
            correct = correct + 1.0
...
if __name__ == '__main__':
    ...
    parser.add_argument('-houzhui', type=str, default="wei_sig(shang_chu50_xia_chu50)_area",
                        help='本次保存路径、曲线路径的后缀')
    ...
    parser.add_argument('-milestones', type=float, default=[0.1, 0.4, 0.6], help='milestones')
    ...
    # Optimzer
    loss_function = nn.BCELoss()  # 这行代码创建了一个二元交叉熵损失函数的实例，用于计算模型输出与真实标签之间的损失。
    ...
    checkpoint_path = (
        os.path.join("{}_{}".format(opt.checkpoint_path, houzhui),
                     'epoch_{epoch}_{train_path}-{type}-acc_{acc}.pth'))
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if epoch > opt.epochs * 0.0 and acc > best_acc:
        # if epoch > opt.epochs * 0.2 and acc > 0.985:
