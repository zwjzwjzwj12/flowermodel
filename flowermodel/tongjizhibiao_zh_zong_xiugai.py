...
if __name__ == '__main__':
    ...
    parser.add_argument('-new_img_path', type=str,
                        default="E:\\aboutjiqixuexi\\zhmx_zong\\makelabel_zh_zong_softmax_test_xiugai_addgls_kuozhongtest",
    ...
    for i in range(15, 29):
        ...
        for file in os.listdir(os.path.join(opt.img_path, "{}".format(i))):  # 从文件夹获取图片数据的文件名
            ...
            fuheresult = [0, float(output_gailv_16), float(output_gailv_17), float(output_gailv_18),
                          ...
                          float(output_gailv_21), pengzhangfenbu(float(output_gailv_22), 0.5), float(output_gailv_23),
                          pengzhangfenbu_24(float(output_gailv_24), 0.5), float(output_gailv_25),
                          float(output_gailv_26), 0, 0]

            if fuheresult[int(result_1)-15] == 0:
                fuheresult[int(result_1)-15] = perc_1
            if fuheresult[int(result_2)-15] == 0:
                fuheresult[int(result_2)-15] = perc_2
            if fuheresult[int(result_3)-15] == 0:
                fuheresult[int(result_3)-15] = perc_3
            ...
            if int(result_1) == 15 or int(result_1) == 27 or int(result_1) == 28:
                obj = Labelper(perc_1, label_array[int(result_1) - 15])
                ...
                while c < 14:
                    if fuhe_result[c].per > fuhe_result[c - 1].per:
                        fuhe_result[c].per = fuhe_result[c - 1].per - 0.01
                    else:
                        break
                    c = c + 1
                if fuhe_result[0].label == "错误功图":
                    for k in range(1, 14):
                        fuhe_result[k].per = fuhe_result[k].per / 4
                # softmax处理
                he = 0.0
